<?php
/**
 * JobScout Customizer Notification Section Class.
 *
 * @package JobScout
 */

/**
 * jobscout_Customizer_Notice_Section class
 */
class jobscout_Customizer_Notice_Section extends WP_Customize_Section {
	...
	public function call_plugin_api( $slug ) {
		...
		$call_api = get_transient( 'jobscout_cust_notice_plugin_info_' . $slug );
		if ( false === $call_api ) {
			...
			set_transient( 'jobscout_cust_notice_plugin_info_' . $slug, $call_api, 30 * MINUTE_IN_SECONDS );
		}
	...
	public function json() {
		$json = parent::json();
		global $jobscout_recommended_plugins;
		...
		$show_recommended_plugins = get_option( 'jobscout_show_recommended_plugins' );

		foreach ( $jobscout_recommended_plugins as $slug => $plugin_opt ) {
			...
			$jobscout_recommended_plugin['url']    = $this->create_action_link( $active['needs'], $slug );
			if ( $active['needs'] !== 'install' && $active['status'] ) {
				$jobscout_recommended_plugin['class'] = 'active';
			} else {
				$jobscout_recommended_plugin['class'] = '';
			}

			switch ( $active['needs'] ) {
				case 'install':
					$jobscout_recommended_plugin['button_class'] = 'install-now button';
					$jobscout_recommended_plugin['button_label'] = $install_button_label;
					break;
				case 'activate':
					$jobscout_recommended_plugin['button_class'] = 'activate-now button button-primary';
					$jobscout_recommended_plugin['button_label'] = $activate_button_label;
					break;
				case 'deactivate':
					$jobscout_recommended_plugin['button_class'] = 'deactivate-now button';
					$jobscout_recommended_plugin['button_label'] = $deactivate_button_label;
					break;
			}
            
			$info   = $this->call_plugin_api( $slug );
			$jobscout_recommended_plugin['id'] = $slug;
			$jobscout_recommended_plugin['plugin_slug'] = $slug;

			if ( ! empty( $plugin_opt['description'] ) ) {
				$jobscout_recommended_plugin['description'] = $plugin_opt['description'];
			} else {
				$jobscout_recommended_plugin['description'] = $info->short_description;
			}

			$jobscout_recommended_plugin['title'] = $info->name;

			$customize_plugins[] = $jobscout_recommended_plugin;
				...
				<div class="recomended-actions_container" id="plugin-filter">
					<# if( data.recommended_plugins.length > 0 ){ #>
						<# for (action in data.recommended_plugins) { #>
							<div class="jobscout-pro-recommeded-actions-container jobscout-pro-recommended-plugins" data-index="{{ data.recommended_plugins[action].index }}">
								<# if( !data.recommended_plugins[action].check ){ #>
									<div class="jobscout-pro-recommeded-actions">
