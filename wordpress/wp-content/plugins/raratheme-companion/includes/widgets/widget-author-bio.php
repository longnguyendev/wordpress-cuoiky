...
// register RaraTheme_Author_Bio widget
function raratheme_register_author_bio_widget() {
    register_widget( 'RaraTheme_Author_Bio' );
}
add_action( 'widgets_init', 'raratheme_register_author_bio_widget' );

function raratheme_author_load_sortable() {    
    wp_enqueue_script( 'jquery-ui-core' );    
    wp_enqueue_script( 'jquery-ui-sortable' );    
}
add_action( 'load-widgets.php', 'raratheme_author_load_sortable' );

if( ! class_exists( 'RaraTheme_Author_Bio' ) ) : 
 /**
 * Adds RaraTheme_Author_Bio widget.
 */
class RaraTheme_Author_Bio extends WP_Widget {

	/**
	 * Register widget with WordPress.
	 */
	function __construct() {
        add_action( 'admin_print_footer_scripts', array( $this,'raratheme_socicon_template' ) );
		parent::__construct(
			'raratheme_author_bio', // Base ID
			__( 'Rara: Author Bio', 'raratheme-companion' ), // Name
			array( 'description' => __( 'An Author Bio Widget', 'raratheme-companion' ), ) // Args
		);
	}
     ...
     * @return string
     */
    function raratheme_get_social_icon_name( $url ) {
        $icon = '';
        $obj = new RaraTheme_Companion_Functions;
        if ( $url = strtolower( $url ) ) {
            foreach ( $this->raratheme_get_supported_url_icon() as $link => $icon_name ) {
                if ( strstr( $url, $link ) ) {
                    $icon = $icon_name;
                }
            }
        ...
        return apply_filters( 'raratheme_social_icons_get_icon_name', $icon, $url );
    }
    ...
    * @since 1.0.0
    */
    function raratheme_socicon_template() { 
    	$screen = get_current_screen();?>
	        <div class="raratheme-socicon-template">
	            <li class="social-share-list" data-id="{{socicon_index}}">
                    <span class="raratheme-social-icons-sortable-handle"></span>
                    <span class="raratheme-social-icons-field-handle"><i class="fas fa-plus"></i></span>
                    <label for="<?php echo esc_attr( $this->get_field_id( 'socicon_profile' ) ); ?>"><?php esc_html_e( 'Social Icon', 'raratheme-companion' ); ?></label>
                    <span class="example-text">Example: facebook</span>
                    <input class="user-social-profile" id="<?php echo esc_attr( $this->get_field_id( 'socicon_profile[{{socicon_index}}]' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'socicon_profile[{{socicon_index}}]' ) ); ?>" type="text" value="" placeholder="<?php _e('Search Social Icons','raratheme-companion');?>" />
                    <label class="link-label" for="<?php echo esc_attr( $this->get_field_id( 'socicon[{{socicon_index}}]' ) ); ?>"><?php esc_html_e( 'Link', 'raratheme-companion' ); ?></label>
	        ...
	        <style type="text/css">.raratheme-socicon-template{display: none;}</style>
    <?php
    }
	...
	public function widget( $args, $instance ) {
        ...
        if ( is_active_widget( false, false, $this->id_base, true ) ) {
            ...
            wp_enqueue_style( 'Dancing-Script', RARATC_FILE_URL .'/public/css/dancing-script.min.css', array(), '1.0.0', 'all' );
        }
        ...
        ?>
        <div class="raratheme-author-bio-holder">
                        ...
                        foreach ( $arr_keys as $key => $value )
                        {
                            if ( array_key_exists( $value, $instance['socicon'] ) )
                            {
                                ...
                                {       
                                    ...
                                    {
                                        $icon = $this->raratheme_get_social_icon_name( $instance['socicon'][$value] );
        ...
        <?php    
        $html = ob_get_clean();
        echo apply_filters( 'rara_author_bio_widget_filter', $html, $args, $instance );
    ...
    public function form( $instance ) {
        ...
        jQuery(document).ready(function($){
            $(".raratheme-sortable-icons").sortable({
                    cursor: "move",
                    update: function (event, ui) {
                        $(".raratheme-sortable-icons .social-share-list input").trigger("change");
                    }
                });
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'name' ) ); ?>"><?php esc_html_e( 'Author Name', 'raratheme-companion' ); ?></label> 
            <input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'name' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'name' ) ); ?>" type="text" value="<?php echo esc_attr( $name ); ?>" />
        </p>
        
        <p>
            <label><?php _e('Display photo from:','raratheme-companion'); ?></label>
            ...
            <label for="<?php echo esc_attr( $this->get_field_id( 'author-image-option' . '-gravatar' ) );?>" class="radio-btn-wrap"><?php _e('Gravatar', 'raratheme-companion');?></label>
            ...
            <label for="<?php echo esc_attr( $this->get_field_id( 'author-image-option' . '-photo' ) );?>" class="radio-btn-wrap"><?php _e('Uploaded Photo','raratheme-companion');?></label>
        </p>
        
        <?php $obj->raratheme_companion_get_image_field( $this->get_field_id( 'author-image' ), $this->get_field_name( 'author-image' ), $author_image, __( 'Upload Author Image', 'raratheme-companion' ) ); ?>
        
        <p class="author-email">
            <label for="<?php echo esc_attr( $this->get_field_id( 'email' ) ); ?>"><?php esc_html_e( 'Author Email', 'raratheme-companion' ); ?></label> 
            <input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'email' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'email' ) ); ?>" type="text" value="<?php echo esc_attr( $email ); ?>" />
        </p>
        <div class="widget-side-note" class="example-text"><?php $link1 = '<a href="http://en.gravatar.com/" rel="noopener noexternal" target="_blank">Gravatar</a>'; echo sprintf( __( 'You can show your %1$s image instead of manually uploading your photo. Just add your gravatar registered email address here.','raratheme-companion'), $link1 );?></div>
        ...
        <p>
            <label><?php _e('Display Signature from:','raratheme-companion'); ?></label>
            ...
            <label for="<?php echo esc_attr( $this->get_field_id( 'author-signature-option' . '-text' ) );?>" class="radio-btn-wrap"><?php _e('Text','raratheme-companion');?></label>
            ...
            <label for="<?php echo esc_attr( $this->get_field_id( 'author-signature-option' . '-photo' ) );?>" class="radio-btn-wrap"><?php _e('Uploaded Photo','raratheme-companion');?></label>
        </p>

        <?php $obj->raratheme_companion_get_image_field( $this->get_field_id( 'image' ), $this->get_field_name( 'image' ), $image, __( 'Upload Signature Image', 'raratheme-companion' ) ); ?>
        ...
        <p class="signature-text">
            ...
            <label for="<?php echo esc_attr( $this->get_field_id( 'signature-text' ) ); ?>"><?php esc_html_e( 'Signature Text', 'raratheme-companion' ); ?></label>
		</p>
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'link' ) ); ?>"><?php esc_html_e( 'Button Link', 'raratheme-companion' ); ?></label>
        ...
        <ul class="raratheme-sortable-icons" id="<?php echo esc_attr( $this->get_field_id( 'raratheme-social-icons' ) ); ?>">
		<?php
                    ...
                    { 
                        if(isset( $instance['socicon'][$value] ) && !empty( $instance['socicon'][$value] ))
                        {
        		            ...
        		            <li class="social-share-list" data-id="<?php echo $value;?>">
        		                <span class="raratheme-social-icons-sortable-handle"></span>
        		                <span class="raratheme-social-icons-field-handle"><i class="<?php echo esc_attr( $class );?>"></i></span>
                                <label for="<?php echo esc_attr( $this->get_field_name( 'socicon_profile['.$value.']' ) ); ?>"><?php esc_html_e( 'Social Icon', 'raratheme-companion' ); ?></label>
                                <span class="example-text">Example: facebook</span>
                                ...
                                <label class="link-label" for="<?php echo esc_attr( $this->get_field_id( 'socicon['.$value.']' ) ); ?>"><?php esc_html_e( 'Link', 'raratheme-companion' ); ?></label>
                                <span class="example-text">Example: http://facebook.com</span>
		...
		?>
        <span class="raratheme-socicon-holder"></span>
        </ul>
		<input type="button" name="button" id="add-user-socicon" class="button button-primary" value="<?php _e('Add Social Profile','raratheme-companion');?>"><br>
        <span class="raratheme-option-side-note" class="example-text"><?php _e('Click on the above button to add social media icons. You can also change the order of the social icons.','raratheme-companion');?></span>
		<?php 
	}
